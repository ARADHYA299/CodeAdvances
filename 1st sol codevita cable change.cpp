...
vector<Pos> tracePath(const vector<vector<string>>& grid, int n, int m, char target) {
    Pos start(-1, -1);
...
int solveCableWrap(int n, int m, vector<vector<string>>& grid) {
    vector<Pos> cablePath = tracePath(grid, n, m, 'C');
    vector<Pos> rodPath = tracePath(grid, n, m, 'R');
    
    if (cablePath.empty() || rodPath.empty()) {
        return 0;
    }
    
    set<pair<int,int>> rodSet;
    for (const Pos& p : rodPath) {
        rodSet.insert({p.row, p.col});
    }
    ...
    for (int i = 0; i < cablePath.size() - 1; i++) {
        ...
        if (rodSet.find({curr.row, curr.col}) != rodSet.end()) {
            ...
            if (crossProduct != 0) {
                char onTop = grid[curr.row][curr.col][0];
                
                if (onTop == 'R') {
                    linkingNumber += (crossProduct > 0) ? 1 : -1;
                } else {
                    linkingNumber += (crossProduct > 0) ? -1 : 1;
                }
            }
